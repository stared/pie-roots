...
- `Sit.tsx` — \*sed-: **one global lattice** (rewritten 2026-07-26, replacing the earlier dated time strata — hairlines and era labels are gone). Every English endpoint owns one column; leaves alternate between two bottom rows so neighbouring words never touch; shared ancestors are centered over their endpoint span, and intermediates keep to their column's row parity (even columns on odd rows, odd columns on even rows) so no two adjacent labels share a line. Each node is a label block — form / ring / gloss / language — and links join block to block: they leave below the language line and stop above the next form, so no line is ever drawn across a word. Two **bundle corridors** are the only structural ornament: the root's six branch heads leave through one, and so do sedēre's eight compounds; siblings share the start of a smooth descent, then separate in source order (bundling, never crossing).
