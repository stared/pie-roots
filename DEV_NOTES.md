...
- `Sit.tsx` — \*sed-: **a checkerboard lattice** (the dated time strata are gone; the layout is by descent, not era). Every English endpoint owns one column, leaves alternating between two bottom rows; **neighbouring columns stagger their intermediates**, which is the point — the checkerboard buys every label the width of two columns, so a gloss can carry the derived sense next to the literal one (obsidēre *sit against; besiege*) instead of being trimmed to fit. Shared ancestors are centered over their endpoint span. Each node is a label block — form / ring / gloss / language — and links join block to block: they leave below the language line and stop above the next form, so no line is drawn across a word. **Routing (2026-07-27): a link descends straight to its child's own column, then drops.** Above the parent's generation line — the depth of its shallowest child — it is one clean diagonal, or a plain vertical for a chain; below it, everything happens inside the child's own column, entered through a soft 13px knee. So a link never travels through a row band it does not belong to, which is what lets the columns stagger freely. **The root is the exception: it distributes through a lintel** — one stem down to a shared horizontal, then a drop into each branch's column — because its ten branches span the full width at mixed depths, and ten diagonals from one point converge into a near-solid wedge (measured: 254px of two links inside 2px of each other). Every other fork is local and takes direct lines. The lintel is drawn once, as a trunk with two rounded ends plus one drop per branch, so no segment is painted twice.
- `Turn.tsx` — \*kʷel-: white ground, four EQUALLY spaced rims (125 each: PIE formations / attested arm bases + first descendants / later intermediates / English), straight limbs fork→rim-leaf with intermediates collinear on the line, symmetric-V forks on the bisector, no hub spokes (lines start at ring 1; contested τῆλε/τέλος sit on ring 2 with dashed stubs), one tight label block per node (above, or below via `flip`), lower-rim English words outside the wheel.
...
- *Dashed means the STEP is uncertain in the sources* — not mere borrowing. Loans drawn flatly by Wiktionary (guide, guise, historia, Veda) are solid; dashes mark hedged derivations (fās, bandito, niche, the two tele-'s, *kʷol-so-, πόλος→poulie). The Hades line is dashed for a different reason: it is not a descent edge at all.
- *Prune nodes that add no information* (Piotr, 2026-07-20): a step whose gloss repeats its neighbor (βουκολικός 'pastoral' beside būcolicus 'pastoral', colōnia 'settlement' before colony) is noise; likewise a reconstructed intermediate inside an already-dashed chain (*πολίδιον) when the surrounding words look alike anyway — the dash carries the doubt, the dek carries the story.
...
- *No bundled fans, no braids* (Piotr, 2026-07-27): routing siblings through a shared corridor looked like one thick wire — `scripts/audit-roots.mjs` measured two root links running within 2px for 329px, and a corridor line crossing both a sibling link and the word *dissidēre*. The fix is the knee routing above, not flattening the layout: **fix the lines, keep the checkerboard.** An earlier attempt put every fork's children on one row so plain straight lines would be safe; it did make the geometry clean, but it cost eleven glosses their second sense and Piotr rejected it as heavy-handed. **Run the audit on every layout change** — it reports links crossing glyph boxes, link crossings, and near-coincident pairs, none of which are visible at page zoom.
- *A word carried only by a contested step, adding no new door, is cut* (Piotr, 2026-07-27, on settle): \*setlaz sits under a rejected-on-phonology attachment, and sit/set already hold the Germanic sit-family, so settle stays in the notes.
- *One English word may carry a gloss: the homograph* (2026-07-27). Modern words stand bare, except where the bare form would be read as a different word entirely — `see` on the sed chart is the bishop's seat, and unglossed it reads as the verb, which belongs to a different root on the neighbouring page. The note disambiguates; it never explains a drift.
- *No invented grouping nodes* (2026-07-27): `*set- / *sat-` "the inherited sit-family" was a convenience — two forms in one node, glossed with a description rather than a meaning. No source posits a shared Proto-Germanic ancestor over \*sitjaną, \*satjaną, \*setiją and \*sōtą, so each now descends from the root directly.
- *Audit the edges you draw, and quote them* (2026-07-27): notes/sed.md carries a per-edge section over the charted subset only — one witness quotation per edge, the hedges that justify each dash, and the corrections the pass caught (\*sētiją→\*setiją, the missing `sedātus`, `assize` mis-drawn as size's ancestor). The research tree above it stays exploration-tier; the line between the two is explicit.
- *Twin stems get their own node, not a dashed shortcut* (Piotr, 2026-07-26, on subsidy): where the dictionaries build a compound on a stem beside the obvious one — Latin sīdō, \*sed- reduplicated, rather than sedēre — the chart draws that stem as its own branch head. Dashing an edge no witness asserts would say "uncertain" when the sources are in fact clear and simply point elsewhere.
...
**Per-root story material (moved from notes/, 2026-07-20 — notes/ files are data + sources only):**

- \*kʷel-: hook — one turning root spins out the wheel, the chakra, the cycle, culture, the colony, the collar, the palindrome — and, if Beekes is right about how far a turn can take you, the telephone. Showpiece: wheel = cycle = chakra as one reduplicated PIE word arriving three ways. Its disputes are architectural (a possible root split in τῆλε, a merger in τέλος, a semantic reconstruction in *kʷol-so-), not leaf-level; English yield roughly double the \*deru- benchmark.
- \*weyd-: a six-armed banyan — the same seen→known change replays independently in every arm, and English plunders all six; Latin videō alone fans into >30 English words; deepest chain ἵστωρ→historia→estoire→story; the Germanic loanback loop (guide, guise) sneaks Germanic material back in dressed as Romance. Star crossover beat: asteroid's -oid IS \*weyd- (εἶδος).
- \*sed-: a many-door tree — no other root enters English through so many independent doors (OE, Norse, French/Latin, Greek coinage, Latin-carried Greek, Irish, Welsh, German, Hebrew-mediated Greek, Sanskrit), plus the PIE-age compound *nisdós 'nest' arriving twice: once by blood (nest), once by borrowing (niche, eyas). The chart draws seven of those doors (Piotr, 2026-07-26): possess, ersatz, eyas, Eisteddfod and Upanishad stay in the notes, so the German, Welsh and Sanskrit doors are story material only — the dek and the blurbs must promise nothing the columns don't show.
- \*bʰeh₂-: three arms, three tricks — Latin turns speech into destiny (fate = 'what has been said'), Greek into sound (phone), Germanic into command (ban); with a built-in impostor root (\*bʰeh₂- 'shine': phase, phantom, fantasy) whose descendants are indistinguishable without the etymology — it lives in the dek only.
