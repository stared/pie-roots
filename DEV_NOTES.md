...
- `Sit.tsx` — \*sed-: **one global lattice, every link straight** (rewritten 2026-07-27; the earlier dated time strata and the bundle corridors that replaced them are both gone). Every English endpoint owns one column; leaves alternate between two bottom rows so neighbouring words never touch; shared ancestors are centered over their endpoint span. Depth is by generation, not era: heads, then one row per descent step. Each node is a label block — form / ring / gloss / language — and links join block to block: they leave below the language line and stop above the next form, so no line is ever drawn across a word. **A fork's children always share a row**, which is what makes straight lines safe: a line to a far child stays above the row it is heading for, so it cannot cut a nearer child's label, and lines leaving one point cannot cross each other. Column order puts the widest forms (supersedēre, subsīdere) where the neighbouring column is a generation away.
...
- *No bundled fans, no braids* (Piotr, 2026-07-27): routing siblings through a shared corridor looked like one thick wire — `scripts/audit-roots.mjs` measured two root links running within 2px for 329px, and a corridor line crossing both a sibling link and the word *dissidēre*. Straight lines from a fork, with its children on one row, cost nothing and cannot do either. **Run the audit on every layout change** — it reports links crossing glyph boxes, link crossings, and near-coincident pairs.
